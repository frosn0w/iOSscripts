...
(function () {
  ...
  function shouldRemovePost(text) {
    ...
    if (month < currentMonth || (currentMonth === 1 && month === 12)) return true;
  ...
  function processDivs(element) {
    ...
    if (id === "main-app-navigation") {
    ...
    } else if (dataset.tag === "creation-name" && textContent.includes("Love & Peace !")) {
      safeRemove(element, null, 4);
      ...
      safeRemove(element);
    } else if (dataset.tag === "comment-body" && textContent.includes("此留言已被移除")) {
        ...
        if (mutation.type === 'childList') {
          mutation.addedNodes.forEach((node) => {
            if (node.nodeType === 1) { // 仅处理元素节点
              const elements = [node, ...node.querySelectorAll("a, button, div, span")];
              elements.forEach(el => {
                  if (el.matches("a")) processLinks(el);
                  if (el.matches("button")) processButtons(el);
                  if (el.matches("div")) processDivs(el);
  ...
  function addWatermarks() {
    ...
    const buttons = document.querySelectorAll('button[data-tag="commenter-name"]');
    buttons.forEach((btn) => {
